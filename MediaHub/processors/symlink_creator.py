...
def process_file(args, processed_files_log, force=False, console=None):
    ...
    if force:
        ...
        if existing_symlink_path:
            if console:
                console(f"Force mode: Found existing symlink at {existing_symlink_path}")
            else:
    ...
    if existing_dest_path and not force:
        if not os.path.exists(existing_dest_path):
            ...
            if os.path.exists(dir_path):
                for filename in os.listdir(dir_path):
                    ...
                    if os.path.islink(potential_new_path) and os.readlink(potential_new_path) == src_file:
                        if console:
                            console(f"Detected renamed file: {existing_dest_path} -> {potential_new_path}")
                        else:
            ...
            if console:
                console(f"File already processed. Source: {src_file}, Existing destination: {existing_dest_path}")
            else:
    ...
    if os.path.islink(dest_file):
        ...
        if existing_src == src_file:
            if console:
                console(f"Symlink already exists and is correct: {dest_file} -> {src_file}")
            else:
            ...
            if console:
                console(f"Updating existing symlink: {dest_file} -> {src_file} (was: {existing_src})")
            else:
    ...
    if os.path.exists(dest_file) and not os.path.islink(dest_file):
        if console:
            console(f"File already exists at destination: {os.path.basename(dest_file)}")
        else:
